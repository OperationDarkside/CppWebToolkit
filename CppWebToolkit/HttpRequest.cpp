...
namespace dnc {
	namespace Web {
		...
		std::string& HttpRequest::Resource () {
			return resource;
		}
		...
		std::optional<std::string> HttpRequest::HeaderField (const std::string & fieldname) {
		...
		std::optional<std::string> HttpRequest::GET (const std::string & fieldname) {
		...
		std::optional<std::string> HttpRequest::POST (const std::string & fieldname) {
		...
		void HttpRequest::Socket (Net::Sockets::Socket socket) {
			sock = socket;
		}
		...
		std::string HttpRequest::UriDecode (const std::string & sSrc) {
		...
		void HttpRequest::ParseGetFields () {
			if (get_str.empty()) {
				return;
			}
		...
		bool HttpRequest::Parse (const std::string & value) {
			...
			std::string header_str = value.substr (first_line_end + 2, header_end_pos);
			size_t header_str_len = header_str.length ();
			size_t new_pos = 0;
			size_t last_pos = 0;
			while ((new_pos = header_str.find ("\r\n", last_pos)) != std::string::npos) {
				line = header_str.substr (last_pos, new_pos);
